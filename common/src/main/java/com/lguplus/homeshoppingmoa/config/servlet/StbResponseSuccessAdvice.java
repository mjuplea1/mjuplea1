package com.lguplus.homeshoppingmoa.config.servlet;
...
@Slf4j
public class StbResponseSuccessAdvice implements ResponseBodyAdvice<Object> {

    private final JacksonMapperConfig jacksonMapper = new JacksonMapperConfig();
    ...
    /*
     * Response OK 응답에 대해서 추가적인 공통 모델을 포함하여 리턴시켜 준다.
     *
     * @param body
    ...
    public Object beforeBodyWrite(Object body, MethodParameter methodParameter, MediaType mediaType, Class<? extends HttpMessageConverter<?>> httpMessageConverter, ServerHttpRequest serverHttpRequest, ServerHttpResponse serverHttpResponse) {
        ...
        if (httpMessageConverter.isAssignableFrom(StringHttpMessageConverter.class)) {
            return this.jacksonMapper.objectMapper().writeValueAsString(new StbCommonModel<>(body));
        }

        return new StbCommonModel<>(body);
    }
